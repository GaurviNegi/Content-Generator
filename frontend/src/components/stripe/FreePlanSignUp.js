...
import { handleFreeSubscription } from "../../api/stripePayment/stripePayment";
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-900 flex justify-center items-center p-6">
      <div className="bg-white rounded-xl shadow-lg p-8 max-w-md w-full">
        <h2 className="text-2xl font-bold text-gray-800 text-center mb-6">
          FreePlanSignup Confirm Your {planDetails.name} Plan
        </h2>

        {/* Placeholder for status message */}
        {mutation.isError && <StatusMessage type="error" message={mutation?.error?.response?.data?.message}/>}
        {mutation.isSuccess && <StatusMessage type="success" message="Subscription Plan successfully upgraded"/>}
        {mutation.isPending && <StatusMessage type="loading" message="Please wait until loading..."/>}
