...
import { stripePaymentIntentAPI } from "../../api/stripePayment/stripePayment";
...
const CheckoutForm = () => {
  ...
  const mutation = useMutation({
    mutationFn: stripePaymentIntentAPI
  });
  ...
  return (
    <div className="bg-gray-900 h-screen -mt-4 flex justify-center items-center">
      ...
      >
        ...
        {mutation?.isError && (<StatusMessage type="error" message={mutation?.error?.reponse?.data?.error}/>)}
