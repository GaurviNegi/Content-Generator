...
import { getContentAPI } from "../api/chatGPT/chatGPT";
import StatusMessage from "../components/alerts/StatusMessage";
import { getUserProfileAPI } from "../api/users/usersAPI";

const BlogPostAIAssistant = () => {
  const [generatedContent, setGeneratedContent] = useState("");

  //useQuery to get the user data
  const { isLoading, isError, data, error , refetch} = useQuery({
    queryKey: ["userProfile"],
    queryFn: getUserProfileAPI,
  });

  //mutation to get the content generation response form the backend
  const mutation = useMutation({
    mutationFn: getContentAPI,
    onSuccess: (data) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      ...
      mutation.mutate(
        `generate a blog post baseed ${values.prompt} ,${values.tone} , ${values.category}`
      );
    },
  });

  //displaying loading based useQuery
  if (isLoading) {
    return (
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-gray-900 flex justify-center items-center p-6">
      <div className="bg-white rounded-xl shadow-xl overflow-hidden max-w-2xl w-full p-6">
        ...
        {/*status messages based on mutation states  */}
        {mutation?.isPending && (
          <StatusMessage
            type="loading"
            message="please wait loading response..."
          />
        )}
        ...
        {mutation?.isSuccess && (
          <StatusMessage
            type="success"
            message="successfull content generation..."
          />
        )}
        ...
        <div className="flex mt-2">
          <div className="mr-2 mb-2">
            <span className="text-sm bg-green-200 font-semibold px-4 py-2 rounded-full">
              {" "}
